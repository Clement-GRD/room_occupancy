...
def plot_one_columns_index_occupancy_compare_transformation (df, column, transformation):
    ...
    axs[1].set_ylabel(f'window_averaged_{column}')
...
def print_metrics(estimator, cross_validate):
    ...
    """
    # calculate accuracy cross validation score
    accuracy_scores = cross_validate['test_accuracy']
    f1_macro_scores = cross_validate['test_f1_macro']

    print(estimator)
    print('metrics\t min\t mean\t max')
    print('A', '\t', np.round(np.min(accuracy_scores),3),'\t', np.round(np.mean(accuracy_scores),3),'\t', np.round(np.max(accuracy_scores), 3))
    print('F1', '\t', np.round(np.min(f1_macro_scores),3),'\t', np.round(np.mean(f1_macro_scores),3),'\t', np.round(np.max(f1_macro_scores), 3))
...
def plot_confusion_acc_f1 (y_true, y_test, title):
    ...
    plt.title(title)
    plt.xlabel('Predicted label\n Accuracy = {:0.3f}  F1-macro = {:0.3f}'\
               .format(accuracy_score(y_true, y_test), f1_score(y_true, y_test, average='macro')))
    plt.plot();
